...
func main() {
	...
	*/
	...
	connStr := "user=postgres dbname=dbApi password=123456 host=127.0.0.1 sslmode=disable"
	...
	db.QueryRow("SELECT to_regclass('public.person')").Scan(&checkDatabase)
	if err != nil {
		fmt.Print(err)
	}
	...
	if checkDatabase == ""{
		...
		createSQL := "CREATE TABLE public.person  (pk SERIAL PRIMARY KEY,fname character varying,lname character varying);"
		db.Query(createSQL)
	}

	//sql to insert employee information
	statement :="INSERT INTO person(fname, lname) VALUES($1, $2)"
	...
	rows, err := db.Query("Select fname, lname from person")
	if err != nil {
		fmt.Print(err)
	}
	defer rows.Close()
